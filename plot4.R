...
par(mfrow = c(2,2), mar = c(7, 5, 2, 1), oma = c(0, 0, 0, 0))
...
with(df2Days, plot(DateTime, Voltage, xlab = "Date & Time", ylab = "Voltage (V)", type = "n"))
with(df2Days, lines(DateTime, Voltage, col = "black"))
...
legend("topright", lty = c(1, 1, 1), col = c("black", "red", "blue"), legend = colnames(df2Days)[7:9], bty = "n", y.intersp = 0.5)

with(df2Days, plot(DateTime, Global_reactive_power, xlab = "Date & Time", ylab = "Global Reactive Power (kilowatts)", type = "n"))
with(df2Days, lines(DateTime, Global_reactive_power, col = "black"))
